...
HalfSuccessFiveAttributes::HalfSuccessFiveAttributes()
{
    m_listName = "Half Success 1 to 5";
    m_displayName = "1 Gold to 5 Bronze\n50% Success Method";
    m_iconPath = "qrc:/Icons/HalfSuccess1to5.png";
    ...
    m_embeddedQMLPath = ":/QML/HalfSuccess1to5.qml";
}

const IdeaAttributes HalfSuccessFiveTilerIdea::attributes()
{
    return Magic<HalfSuccessFiveAttributes>::cast();
}

HalfSuccessFiveTilerIdea::HalfSuccessFiveTilerIdea() :
    m_speed(1000.0)
{
    ...
    connect(m_timer, &QTimer::timeout, this, &HalfSuccessFiveTilerIdea::step);

    restart();
}

std::shared_ptr<DataType> HalfSuccessFiveTilerIdea::dataOut(PlugNumber)
{
    return std::dynamic_pointer_cast<DataType>(m_tiles);
}

void HalfSuccessFiveTilerIdea::onNewDataIn(std::shared_ptr<DataType> dataType, PlugNumber)
{
    ...
    if(n)
    {
        ...
        connect(n->getModel(),&CoinArrayModel::coinsAdded,this, &HalfSuccessFiveTilerIdea::coinsAdded);
        connect(n->getModel(),&CoinArrayModel::coinsCleared, this, &HalfSuccessFiveTilerIdea::coinsCleared);
...
void HalfSuccessFiveTilerIdea::coinsAdded()
{
    step();
    m_timer->start();
}

void HalfSuccessFiveTilerIdea::coinsCleared()
{
    restart();
    m_tiles->getModel()->clearTiles();
}

void HalfSuccessFiveTilerIdea::step()
{
    auto n = m_input.lock();
...
void HalfSuccessFiveTilerIdea::restart()
{
    emit reset();
    m_addCoinChoiceStep = false;
    m_botMoveStep = false;
...
void HalfSuccessFiveTilerIdea::addCoinChoice()
{
    emit showNextChoice(105.000 + 79*m_coinsCounter, 560 - 560 * m_top,
                        79.0, 560 * (m_top - m_bot));
}

void HalfSuccessFiveTilerIdea::addTile()
{
    if(m_addGold)
    {
...
void HalfSuccessFiveTilerIdea::botMove()
{
    emit forwardMoveBot(m_botXGoal, m_botYGoal);
}

double HalfSuccessFiveTilerIdea::speed() const
{
    return m_speed;
}

void HalfSuccessFiveTilerIdea::setSpeed(double newSpeed)
{
    if (m_speed == newSpeed)
        return;
