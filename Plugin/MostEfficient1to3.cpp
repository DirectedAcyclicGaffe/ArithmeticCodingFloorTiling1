...
MostEfficient1to3Attributes::MostEfficient1to3Attributes()
{
    m_listName = "Most Efficient 1 to 3";
    m_displayName = "1 Gold to 3 Bronze\nMost Efficient";
    m_iconPath = "qrc:/Icons/MostEfficient.png";
    ...
    m_embeddedQMLPath = ":/QML/MostEfficient1to3.qml";
}

const IdeaAttributes MostEfficient1to3TilerIdea::attributes()
{
    return Magic<MostEfficient1to3Attributes>::cast();
}

MostEfficient1to3TilerIdea::MostEfficient1to3TilerIdea() :
    m_speed(1000.0)
{
    ...
    connect(m_timer, &QTimer::timeout, this, &MostEfficient1to3TilerIdea::step);

    restart();
}

std::shared_ptr<DataType> MostEfficient1to3TilerIdea::dataOut(PlugNumber)
{
    return std::dynamic_pointer_cast<DataType>(m_tiles);
}

void MostEfficient1to3TilerIdea::onNewDataIn(std::shared_ptr<DataType> dataType, PlugNumber)
{
    ...
    if(n)
    {
        ...
        connect(n->getModel(),&CoinArrayModel::coinsAdded,this, &MostEfficient1to3TilerIdea::coinsAdded);
        connect(n->getModel(),&CoinArrayModel::coinsCleared, this, &MostEfficient1to3TilerIdea::coinsCleared);
...
void MostEfficient1to3TilerIdea::coinsAdded()
{
    step();
    m_timer->start();
}

void MostEfficient1to3TilerIdea::coinsCleared()
{
    restart();
    m_tiles->getModel()->clearTiles();
}

void MostEfficient1to3TilerIdea::step()
{
    auto n = m_input.lock();
    ...
    {
        m_addTileStep = false;
        emit addTileChoice(m_rightBotXGoal + 11, 560 * (1.0 - m_rightTop),
                           21.0, 560.0*(m_rightTop - m_rightBot));
        addTile();
        return;
    }
    ...
    if(m_leftTop <= m_rightMid) // Can we add a bronze tile?
    {
        ...
        m_rightBotXGoal = 105.0  + 10 + 21.0 * m_tileCounter;
...
void MostEfficient1to3TilerIdea::restart()
{
    emit reset();
    m_leftBotMoveStep = false;
    m_rightBotMoveStep = false;
...
void MostEfficient1to3TilerIdea::leftBotMove()
{
    emit leftBotMove(m_leftBotXGoal, m_leftBotYGoal);
}

void MostEfficient1to3TilerIdea::rightBotMove()
{
    emit rightBotMove(m_rightBotXGoal, m_rightBotYGoal);
}

void MostEfficient1to3TilerIdea::zoomStep()
{
    ...
    emit zoom(21.0 * m_coinsCounter, 21.0 * m_tileCounter, qPow(2.0, m_coinsCounter),
...
double MostEfficient1to3TilerIdea::speed() const
{
    return m_speed;
}

void MostEfficient1to3TilerIdea::setSpeed(double newSpeed)
{
    if (m_speed == newSpeed)
        return;
